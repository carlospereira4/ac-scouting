...
/* ─── Image compression ────────────────────────────────────────── */
function compressImage(file: File, maxPx = 400, quality = 0.65): Promise<string> {
...
function ScoutingApp() {
  ...
  const doSave = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (player.firestoreId) {
        ...
        const docRef = await addDoc(col, { ...saveData, criadoEm: serverTimestamp() })
        setPlayers(ps => ps.map(p => p.id === id ? { ...p, firestoreId: docRef.id } : p))
      }
  ...
  /* Photo — compress and store in Firestore */
  ...
  const downloadPDF = useCallback(async (reports: SavedReport[], filename: string) => {
    ...
    const pdfDoc = new (jsPDF as any)({ unit: 'mm', format: 'a4' })
    for (let i = 0; i < reports.length; i++) {
      if (i > 0) pdfDoc.addPage()
      await buildPDF(reports[i], pdfDoc)
    }
  ...
  if (authLoading || !user) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100dvh', color: 'var(--text-dim)', fontFamily: "'Barlow', sans-serif", fontSize: 13 }}>
        A carregar…
      </div>
    )
  }
      ...
      <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', height: 58, borderBottom: '1px solid var(--border)', background: 'rgba(10,14,24,.97)', flexShrink: 0, position: 'relative' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 4px 4px 10px', borderRadius: 20, background: 'var(--surface2)', border: '1px solid var(--border2)' }}>
            {user.photoURL
              ? <img src={user.photoURL} style={{ width: 26, height: 26, borderRadius: '50%', objectFit: 'cover' }} alt="" referrerPolicy="no-referrer" />
              : <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'var(--ac-blue)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: '#fff' }}>
            ...
            <button
              ...
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = 'var(--grade-e)'; (e.currentTarget as HTMLElement).style.borderColor = 'var(--grade-e)' }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = 'var(--text-muted)'; (e.currentTarget as HTMLElement).style.borderColor = 'var(--border2)' }}
            >
              Sair
            </button>
          </div>
                ...
                <>
                  ...
                  <Section label="Foto do Jogador">
                    <label style={{ border: '1.5px dashed var(--border2)', borderRadius: 7, padding: '12px 14px', cursor: 'pointer', background: 'var(--surface)', position: 'relative', display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div>
                        <strong style={{ display: 'block', fontSize: 12.5, color: 'var(--text)', marginBottom: 2 }}>
                          {active.photo ? 'Alterar foto' : 'Carregar foto'}
                        </strong>
                        <span style={{ fontSize: 11, color: 'var(--text-dim)' }}>
                          JPG ou PNG — comprimida e partilhada com todos
                        </span>
                      </div>
                    </label>
                  </Section>
                  ...
                  <Section label="Informações do Jogador">
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9 }}>
                      ...
                      <Field label="Posição Específica em Campo" full><input value={active.posicaoEspecifica} onChange={e => updateActive('posicaoEspecifica', e.target.value)} placeholder="ex: LD — Lateral Direito, EE — Extremo Esquerdo" style={inputStyle} /></Field>
          ...
          >
            ...
            <div style={{ padding: 13 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 11, marginBottom: 9 }}>
                <div style={{ width: 44, height: 44, borderRadius: 6, flexShrink: 0, overflow: 'hidden', background: 'var(--surface2)', border: '2px solid var(--border2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 800, fontSize: 15, color: 'var(--text-dim)' }}>
                  {latest.photo
                    ? <img src={latest.photo} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" />
                    : ini
                  }
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {latest.criadoPor && <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 1 }}>por {latest.criadoPor}</div>}
                </div>
    ...
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 16px', background: 'var(--surface2)', borderBottom: '1px solid var(--border)', flexWrap: 'wrap', gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' }}>
          ...
          {r.criadoPor && <Tag type="date">👤 {r.criadoPor}</Tag>}
        </div>
    ...
    <div style={{ background: '#fff', borderRadius: 3, overflow: 'hidden', boxShadow: '0 10px 50px rgba(0,0,0,.55)', color: '#111', fontFamily: "'Barlow', Arial, sans-serif" }}>
      ...
      <div style={{ background: '#f5f7fc', borderBottom: '4px solid', borderImage: 'linear-gradient(90deg, #d0021b 50%, #003f8a 50%) 1', padding: '13px 18px', display: 'flex', gap: 13 }}>
        <div style={{ width: 58, height: 58, borderRadius: 5, overflow: 'hidden', flexShrink: 0, border: '2.5px solid #d0021b', background: '#dde4f2', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 800, fontSize: 18, color: '#8899bb' }}>
          {photoSrc ? <img src={photoSrc} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" /> : ini}
        </div>
        <div style={{ flex: 1 }}>
